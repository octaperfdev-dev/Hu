...
import { db, handleFirestoreError, OperationType, collection, query, where, getDocs, doc, setDoc, deleteDoc, onSnapshot, updateDoc, initializeApp, deleteApp, getAuth, createUserWithEmailAndPassword, firebaseConfig, signOut, writeBatch, getCountFromServer, orderBy, limit, startAfter, endBefore, limitToLast } from '../firebase';
...
export default function UserManagement() {
  ...
  const handlePageChange = (page: number, dir: 'next' | 'prev' | null) => {
    setDirection(dir);
    setCurrentPage(page);
  };
    ...
    <div className="space-y-8 px-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-1">
            <span className="text-xs font-bold text-slate-500 uppercase">Page {currentPage} of {Math.ceil(totalCount / pageSize) || 1}</span>
            <div className="flex items-center gap-1 border-l border-slate-100 ml-2 pl-2">
              <button 
                onClick={() => handlePageChange(currentPage - 1, 'prev')}
                disabled={currentPage === 1 || loading}
                className="p-1 hover:bg-slate-50 rounded text-slate-500 disabled:opacity-30"
              >
                <ChevronLeft size={18} />
              </button>
              <button 
      ...
      <div className="flex items-center justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div className="text-sm text-slate-500">
          Showing <span className="font-bold text-slate-900">{(currentPage - 1) * pageSize + 1}</span> to <span className="font-bold text-slate-900">{Math.min(currentPage * pageSize, totalCount)}</span> of <span className="font-bold text-slate-900">{totalCount}</span> users
        </div>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => handlePageChange(1, null)}
            disabled={currentPage === 1 || loading}
            ...
            {[...Array(Math.min(5, Math.ceil(totalCount / pageSize)))].map((_, i) => {
              ...
              return (
                <button
                  key={pageNum}
                  onClick={() => {
                    if (pageNum > currentPage) handlePageChange(pageNum, 'next');
                    else if (pageNum < currentPage) handlePageChange(pageNum, 'prev');
                  }}
                  disabled={loading}
          ...
          <button 
            onClick={() => handlePageChange(Math.ceil(totalCount / pageSize), 'next')}
            disabled={currentPage >= Math.ceil(totalCount / pageSize) || loading}
            className="px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50 rounded-xl disabled:opacity-30 transition-colors"
          >
            Last
